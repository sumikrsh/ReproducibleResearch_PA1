...
```
Mean total number of steps taken per day is `r totalmean`.
...
```
Median total number of steps taken per day is `r totalmedian`.
...
int <- totals[x,][1]
```
5-minute interval, on average across all the days in the dataset, which contains the maximum number of steps is `r int`.
...
```
Total number of missing values in the dataset is `r numNA`. 
...
totalmean <- mean(totals[,2])
```
Mean total number of steps taken per day (from imputed data) is `r totalmean`.
...
totalmedian <- median(totals[,2])
```
Median total number of steps taken per day (from imputed data) is `r totalmedian`.
